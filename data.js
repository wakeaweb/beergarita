// data.js — The Beergarita Pub
// Sayfa açılışında api.beergarita.com.tr/api/v1 üzerinden veri çeker.
// API erişilemezse aşağıdaki sabit veriler fallback olarak kullanılır.
(function () {

  var API_HOST = 'https://api.beergarita.com.tr';
  ...
  var FALLBACK = {
    ...
    reviewsSummary: {
      average: 4.8,
      count: 124,
      googleLinkLabel: 'Google Yorumlar',
      googleLinkUrl: 'https://tinyurl.com/yktn4hy4'
    }
  };

  // ------- API VERİ DÖNÜŞTÜRÜCÜLER -------

  // API venue verisini window.PUB formatına dönüştür
  function mapVenue(apiVenue) {
    return {
      hours: (apiVenue.hours || []).map(function (h) {
        return { d: h.d, h: h.h };
      }),
      contact: {
        address: apiVenue.address || FALLBACK.contact.address,
        phone: apiVenue.phone || FALLBACK.contact.phone,
        phoneHref: apiVenue.phone_href || FALLBACK.contact.phoneHref,
        maps: apiVenue.maps_embed || FALLBACK.contact.maps,
        mapsLink: apiVenue.maps_link || FALLBACK.contact.mapsLink,
        social: (apiVenue.social || []).map(function (s) {
          return { label: s.label, handle: s.handle || s.label, href: s.href || s.url || '#' };
        })
      }
    };
  }

  // API menu verisini window.PUB.menu formatına dönüştür
  function mapMenu(apiMenu) {
    return (apiMenu || []).map(function (cat) {
      return {
        id: cat.id,
        name: cat.name,
        kicker: cat.kicker || '',
        // API'den gelen URL'i direkt img olarak sakla (artık key değil tam URL)
        img: cat.img || '',
        _isApiUrl: true, // render kısmı URL mi key mi anlaması için
        items: (cat.items || []).map(function (item) {
          return {
            n: item.n,
            d: item.d || '',
            p: item.p || '',
            popupImg: item.popupImg || undefined,
  ...
  // API events verisini window.PUB.events formatına dönüştür
  function mapEvents(apiEvents) {
    return (apiEvents || []).map(function (ev) {
      return {
        day: ev.day,
        date: ev.date,
        time: ev.time,
  ...
  // API gallery verisini window.PUB.gallery formatına dönüştür (string URL dizisi)
  function mapGallery(apiGallery) {
    return apiGallery || [];
  }

  // API reviews verisini window.PUB formatına dönüştür
  function mapReviews(apiReviews) {
    return {
      reviews: (apiReviews.reviews || []).map(function (r) {
        return { n: r.n, s: r.s, t: r.t };
      }),
      reviewsSummary: {
        average: apiReviews.average || FALLBACK.reviewsSummary.average,
        count: apiReviews.count || FALLBACK.reviewsSummary.count,
        googleLinkLabel: apiReviews.google_link_label || FALLBACK.reviewsSummary.googleLinkLabel,
        googleLinkUrl: apiReviews.google_link_url || FALLBACK.reviewsSummary.googleLinkUrl
      }
    };
  }

  // ------- RENDER TETIKLEYICI -------
  // Tüm fetch tamamlandığında veya fallback'e düşüldüğünde bu çağrılır
  function dispatchReady() {
    var ev = new Event('pubDataReady');
  ...
  function fetchFromApi() {
    var endpoints = [
      API_HOST + '/api/v1/venue',
      API_HOST + '/api/v1/menu',
      API_HOST + '/api/v1/events',
      API_HOST + '/api/v1/gallery',
      API_HOST + '/api/v1/reviews'
    ];
    ...
    Promise.all(endpoints.map(fetchPromise))
      .then(function (results) {
        var venueData    = results[0];
        var menuData     = results[1];
        var eventsData   = results[2];
        var galleryData  = results[3];
        var reviewsData  = results[4];

        var venueFormatted  = mapVenue(venueData);
        var reviewsFormatted = mapReviews(reviewsData);

        // window.PUB'u API verisiyle güncelle, video/poster/images sabit kalır
        window.PUB.hours          = venueFormatted.hours;
        window.PUB.contact        = venueFormatted.contact;
        window.PUB.menu           = mapMenu(menuData);
        window.PUB.events         = mapEvents(eventsData);
        window.PUB.gallery        = mapGallery(galleryData);
        window.PUB.reviews        = reviewsFormatted.reviews;
        window.PUB.reviewsSummary = reviewsFormatted.reviewsSummary;
        window.PUB._fromApi       = true;

        dispatchReady();
      })
      .catch(function (err) {
        console.warn('[data.js] API erişilemedi, fallback statik veri kullanılıyor.', err);
        // window.PUB zaten FALLBACK ile dolu, doğrudan render
        dispatchReady();
      });
  }

  // ------- BAŞLANGIÇ: STATIK VERİYİ HEMEN YÜKle, SONRA API'DEN GÜNCELLE -------
  window.PUB = {
    ...
    reviewsSummary: FALLBACK.reviewsSummary,
    _fromApi:       false
  };

  // API fetch'i başlat (async — sonuç gelince pubDataReady eventi fırlatır)
  fetchFromApi();
